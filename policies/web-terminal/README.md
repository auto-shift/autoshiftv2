# web-terminal AutoShift Policy

## Overview
This policy installs the web-terminal operator using AutoShift patterns.
...
helm template policies/web-terminal/
```
...
hubClusterSets:
  hub:
    labels:
      ...
      # web-terminal-version: 'web-terminal.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      web-terminal: 'true'
      web-terminal-channel: 'fast'  # Override channel for this cluster
```
...
- **Version Pinning**: Add `web-terminal-version` label to pin to a specific CSV version
...
```yaml
web-terminal-version: 'web-terminal.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests web-terminal -o jsonpath='{.status.channels[*].currentCSV}'
```
...
#### Common Patterns:
- `policy-web-terminal-config.yaml` - Main configuration
- `policy-web-terminal-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-web-terminal-config" }}
{{- $placementName := "placement-policy-web-terminal-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-web-terminal-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
            ...
            - complianceType: musthave
              objectDefinition:
                ...
                spec:
                  ...
                  # setting: '{{ "{{hub" }} index .ManagedClusterLabels "autoshift.io/web-terminal-setting" | default "default-value" {{ "hub}}" }}'
...
# web-terminal-setting<string>: Configuration option (default: 'value')
# web-terminal-feature-enabled<bool>: Enable optional feature (default: 'false')
# web-terminal-provider<string>: Provider-specific config (default: 'generic')
...
3. Check policy status: `oc describe policy policy-web-terminal-operator-install`
...
1. Test locally: `helm template policies/web-terminal/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/web-terminal/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/web-terminal) - Find your operator details
