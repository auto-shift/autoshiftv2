...
Configure standalone ethernet interfaces. When a MAC address label is present, the policy will use `identifier: mac-address` so NMState matches the physical NIC by MAC rather than by interface name. This is recommended for bare-metal nodes where interface names may not be stable across reboots.

| Label | Description | Example |
|-------|-------------|---------|
| `nmstate-ethernet-{id}` | Interface name (used as profile name when MAC is set) | `eno1` |
| `nmstate-ethernet-{id}-mac` | MAC address (use dots) — enables MAC-based matching | `aa.bb.cc.dd.ee.ff` |
| `nmstate-ethernet-{id}-mtu` | MTU size | `9000` |
| `nmstate-ethernet-{id}-state` | Interface state | `up` (default), `down` |

> **Note:** When `nmstate-ethernet-{id}-mac` is set, the policy renders `identifier: mac-address` in the NNCP. NMState will match the interface by MAC address and use the `name` value as the profile name in NetworkManager. If no MAC is set, matching falls back to interface name (existing behaviour, unchanged).
...
For scenarios where different nodes need different configurations (e.g., different static IPs or per-host MAC addresses), use the `nmstate-host-{id}` prefix. Each host identifier `{id}` creates a separate NNCP targeting that specific hostname.
...
| `nmstate-host-{id}-ethernet-{id}` | Ethernet interface name (profile name) | `eno1` |
| `nmstate-host-{id}-ethernet-{id}-mac` | MAC address (use dots) — enables MAC-based matching | `aa.bb.cc.dd.ee.ff` |
| `nmstate-host-{id}-ethernet-{id}-mtu` | MTU size | `9000` |
| `nmstate-host-{id}-ethernet-{id}-state` | Interface state | `up` (default) |
...
```yaml
# Correct
nmstate-ethernet-nic1-mac: 'aa.bb.cc.dd.ee.ff'

# Incorrect (will fail)
nmstate-ethernet-nic1-mac: 'aa:bb:cc:dd:ee:ff'
```

The policy automatically converts dots to colons when generating the NNCP.
...
Kubernetes labels cannot contain colons (`:`), so MAC addresses must be specified using dots (`.`) as separators when writing labels manually:
...
The policy automatically converts dots to colons when generating the NNCP. When using the label generator script, MACs can be written with colons in the values file — the conversion is handled automatically.
