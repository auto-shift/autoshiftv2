...
- Replicate a ServiceNow credential secret from a hub-cluster source namespace into the KVA target namespace on each managed cluster
...
| `templates/policy-kva-secret.yml` | `Policy`, `Placement`, `PlacementBinding` | Copies KVA credentials from a hub source namespace into the managed cluster KVA namespace |
...
| `kvaHubSecretNamespace` | `cluster-install-secrets` | Default source namespace on the hub cluster for KVA credentials replicated to managed clusters |
...
- KVA credential secret exists on the hub cluster in `kvaHubSecretNamespace` (or `kva.secret-namespace` override from rendered-config), containing:
  - `.user`
  - `.password`
...
| `kva.secret-name` | hub source secret name used by the secret replication policy | `k8s-informer-cred-<servicenow.instance.name>` |
| `kva.secret-namespace` | hub source secret namespace used by the secret replication policy | `cluster-install-secrets` |
...
The secret policy resolves `kva.secret-name` and `kva.secret-namespace` using hub templates, then reads `.user` and `.password` from that hub secret and writes those values into the target secret on each matching managed cluster.
