...
flowchart TB
  ...
  S --> CN["same CN contract in all minting modes:<br/>&lt;certCNPrefix&gt;.&lt;managedClusterName&gt;.&lt;baseDomain&gt;<br/>(cluster segment truncated to fit 63 chars;<br/>collisions detected → loud failure)"]
  E --> CN
...
- **Same CN derivation on hub and spoke.** In `externalCA` mode the spoke derives the CN from
  `$.ManagedClusterName` exactly as the hub derives the RBAC subject, so identity and
  authorization match without any cert crossing clusters.
