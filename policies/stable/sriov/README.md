# sriov AutoShift Policy

## Overview
This policy installs the sriov-network-operator operator using AutoShift patterns.
...
hubClusterSets:
  hub:
    labels:
      ...
      # sriov-version: 'sriov-network-operator.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      sriov: 'true'
      sriov-channel: 'fast'  # Override channel for this cluster
```
...
New policies are automatically discovered by the ApplicationSet. In Git mode, the ApplicationSet globs `policies/stable/*`, `policies/certified/*`, and `policies/community/*` to pick up all subdirectories. No manual registration is required — simply adding your policy folder under the appropriate category (`stable`, `certified`, or `community`) is sufficient.
...
- **Version Pinning**: Add `sriov-version` label to pin to a specific CSV version
...
```yaml
sriov-version: 'sriov-network-operator.v1.x.x'
```

Find available CSV versions:
```bash
# List available versions for this operator
oc get packagemanifests sriov-network-operator -o jsonpath='{.status.channels[*].currentCSV}'
```
...
#### Common Patterns:
- `policy-sriov-config.yaml` - Main configuration
- `policy-sriov-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-sriov-config" }}
{{- $placementName := "placement-policy-sriov-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-sriov-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
          ...
          object-templates:
            - complianceType: musthave
              objectDefinition:
                ...
                metadata:
                  name: sriov-config
                  namespace: {{ .Values.sriov.namespace }}
                spec:
                  # Your operator-specific configuration
                  # Use dynamic labels when needed:
                  # setting: '{{ "{{hub" }} index .ManagedClusterLabels "autoshift.io/sriov-setting" | default "default-value" {{ "hub}}" }}'
...
# sriov-setting<string>: Configuration option (default: 'value')
# sriov-feature-enabled<bool>: Enable optional feature (default: 'false')
# sriov-provider<string>: Provider-specific config (default: 'generic')
...
3. Check policy status: `oc describe policy policy-sriov-operator-install`

### Operator Installation Issues
1. Check subscription: `oc get subscription -n openshift-sriov-network-operator`
2. Check install plan: `oc get installplan -n openshift-sriov-network-operator`
3. Verify operator source exists: `oc get catalogsource -n openshift-marketplace`

### Template Rendering Issues
1. Test locally: `helm template policies/sriov/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/sriov/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/sriov-network-operator) - Find your operator details
