...
file-integrity/
  ...
  placement.yaml                # Placement predicate (autoshift.io/file-integrity) + tolerations
  manifests/
    operator-install/
...
KUSTOMIZE_PLUGIN_HOME=$PWD/.tools/kustomize-plugin .tools/kustomize build \
  ...
  policies/stable/file-integrity
...
`autoshift.io/file-integrity: 'true'`.
...
hubClusterSets:
  hub:
    labels:
      ...
      # file-integrity-version: 'file-integrity-operator.v1.x.x'  # Optional: pin to a specific CSV
```
...
- **Pin a version** with the `file-integrity-version` label — a pinned version sets a manual
  install plan (`startingCSV` + `versions`), so upgrades become explicit.
```bash
# Find available CSV versions
oc get packagemanifests file-integrity-operator -o jsonpath='{.status.channels[*].currentCSV}'
```
     ...
     - complianceType: musthave
       objectDefinition:
         ...
         metadata:
           ...
           namespace: openshift-file-integrity
         spec:
           setting: '{{hub index .ManagedClusterLabels "autoshift.io/file-integrity-setting" | default "default-value" hub}}'
   ```
4. If the config must be its **own** Policy (separate remediation, a dependency, or a different
   ...
   `policy-file-integrity-operator-install`) and point it at the new manifest with its own
   `placement.placementPath`. `generate-policy.sh --dir policies/stable/file-integrity` scaffolds this.

Document any new `autoshift.io/file-integrity-*` label in `autoshift/values/clustersets/_example.yaml`
...
- **Policy not applied**: `oc get managedcluster <cluster> --show-labels`; check the Placement
  (`oc get placement -n policies-autoshift`) and `oc describe policy policy-file-integrity-operator-install`.
- **Operator install issues**: `oc get subscription,installplan -n openshift-file-integrity`;
...
## Resources
- [Operator Documentation](https://operatorhub.io/operator/file-integrity-operator)
