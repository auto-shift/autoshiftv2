# container-security AutoShift Policy

## Overview
Installs the container-security-operator operator using AutoShift's ACM **PolicyGenerator** pattern. The
...
container-security/
  ...
  placement.yaml                # Placement predicate (autoshift.io/container-security) + tolerations
  manifests/
    operator-install/
...
KUSTOMIZE_PLUGIN_HOME=$PWD/.tools/kustomize-plugin .tools/kustomize build \
  ...
  policies/stable/container-security
...
`autoshift.io/container-security: 'true'`.
...
hubClusterSets:
  hub:
    labels:
      ...
      container-security-subscription-name: 'container-security-operator'
      container-security-channel: 'stable-3.17'
      container-security-source: 'redhat-operators'
      container-security-source-namespace: 'openshift-marketplace'
      # container-security-version: 'container-security-operator.v1.x.x'  # Optional: pin to a specific CSV
```
...
- **Pin a version** with the `container-security-version` label — a pinned version sets a manual
  install plan (`startingCSV` + `versions`), so upgrades become explicit.
```bash
# Find available CSV versions
oc get packagemanifests container-security-operator -o jsonpath='{.status.channels[*].currentCSV}'
```
     ...
     - complianceType: musthave
       objectDefinition:
         ...
         metadata:
           ...
           namespace: openshift-operators
         spec:
           setting: '{{hub index .ManagedClusterLabels "autoshift.io/container-security-setting" | default "default-value" hub}}'
   ```
4. If the config must be its **own** Policy (separate remediation, a dependency, or a different
   ...
   `policy-container-security-operator-install`) and point it at the new manifest with its own
   `placement.placementPath`. `generate-policy.sh --dir policies/stable/container-security` scaffolds this.

Document any new `autoshift.io/container-security-*` label in `autoshift/values/clustersets/_example.yaml`
...
- **Policy not applied**: `oc get managedcluster <cluster> --show-labels`; check the Placement
  (`oc get placement -n policies-autoshift`) and `oc describe policy policy-container-security-operator-install`.
- **Operator install issues**: `oc get subscription,installplan -n openshift-operators`;
...
## Resources
- [Operator Documentation](https://operatorhub.io/operator/container-security-operator)
