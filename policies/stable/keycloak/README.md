...
## Overview
This policy installs the rhbk-operator operator using AutoShift patterns.
...
helm template policies/keycloak/
```
...
hubClusterSets:
  hub:
    labels:
      ...
      # keycloak-version: 'rhbk-operator.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      keycloak: 'true'
      keycloak-channel: 'fast'  # Override channel for this cluster
```
...
- **Version Pinning**: Add `keycloak-version` label to pin to a specific CSV version
...
```yaml
keycloak-version: 'rhbk-operator.v1.x.x'
```
...
#### Common Patterns:
- `policy-keycloak-config.yaml` - Main configuration
- `policy-keycloak-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-keycloak-config" }}
{{- $placementName := "placement-policy-keycloak-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-keycloak-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
...
# keycloak-setting<string>: Configuration option (default: 'value')
# keycloak-feature-enabled<bool>: Enable optional feature (default: 'false')
# keycloak-provider<string>: Provider-specific config (default: 'generic')
...
3. Check policy status: `oc describe policy policy-keycloak-operator-install`

### Operator Installation Issues
1. Check subscription: `oc get subscription -n keycloak-system`
2. Check install plan: `oc get installplan -n keycloak-system`
3. Verify operator source exists: `oc get catalogsource -n openshift-marketplace`

### Template Rendering Issues
1. Test locally: `helm template policies/keycloak/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/keycloak/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/rhbk-operator) - Find your operator details
