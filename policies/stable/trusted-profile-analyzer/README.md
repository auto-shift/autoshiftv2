...
## Overview
This policy installs the rhtpa-operator operator using AutoShift patterns.
...
helm template policies/trusted-profile-analyzer/
```
...
hubClusterSets:
  hub:
    labels:
      ...
      # trusted-profile-analyzer-version: 'rhtpa-operator.v1.x.x'  # Optional: pin to specific CSV version
...
clusters:
  my-cluster:
    labels:
      trusted-profile-analyzer: 'true'
      trusted-profile-analyzer-channel: 'fast'  # Override channel for this cluster
```
...
- **Version Pinning**: Add `trusted-profile-analyzer-version` label to pin to a specific CSV version
...
```yaml
trusted-profile-analyzer-version: 'rhtpa-operator.v1.x.x'
```
...
#### Common Patterns:
- `policy-trusted-profile-analyzer-config.yaml` - Main configuration
- `policy-trusted-profile-analyzer-<feature>.yaml` - Feature-specific configs

#### Template Structure:
```yaml
{{- $policyName := "policy-trusted-profile-analyzer-config" }}
{{- $placementName := "placement-policy-trusted-profile-analyzer-config" }}
...
spec:
  disabled: false
  dependencies:
    - name: policy-trusted-profile-analyzer-operator-install
      namespace: {{ .Values.policy_namespace }}
      apiVersion: policy.open-cluster-management.io/v1
          ...
          object-templates:
            - complianceType: musthave
              objectDefinition:
                ...
                metadata:
                  name: trusted-profile-analyzer-config
                  namespace: {{ .Values.trustedProfileAnalyzer.namespace }}
                spec:
...
spec:
  ...
  predicates:
    - requiredClusterSelector:
        labelSelector:
          matchExpressions:
            - key: 'autoshift.io/trusted-profile-analyzer'
              operator: In
              values:
              - 'true'
  tolerations:
...
# trusted-profile-analyzer-setting<string>: Configuration option (default: 'value')
# trusted-profile-analyzer-feature-enabled<bool>: Enable optional feature (default: 'false')
# trusted-profile-analyzer-provider<string>: Provider-specific config (default: 'generic')
...
- objectDefinition:
    ...
    kind: ConfigurationPolicy
    metadata:
      name: trusted-profile-analyzer-csv-status
    spec:
...
3. Check policy status: `oc describe policy policy-trusted-profile-analyzer-operator-install`

### Operator Installation Issues
1. Check subscription: `oc get subscription -n trusted-profile-analyzer`
2. Check install plan: `oc get installplan -n trusted-profile-analyzer`
3. Verify operator source exists: `oc get catalogsource -n openshift-marketplace`

### Template Rendering Issues
1. Test locally: `helm template policies/trusted-profile-analyzer/`
2. Check hub escaping: Look for `{{ "{{hub" }} ... {{ "hub}}" }}` patterns
3. Validate YAML: `helm lint policies/trusted-profile-analyzer/`

## Resources
- [Operator Documentation](https://operatorhub.io/operator/rhtpa-operator) - Find your operator details
